...
export function DailyReportEvidenceDetails({ viewer, report, attachments }: Props) {
  ...
  return <ul aria-label={t('daily.visibleEvidence')} className="daily-report-evidence-details">{visibleEvidence.map((item) => <li key={item.id}>{item.label}</li>)}{visibleAttachments.map((item) => <li key={item.id}>{item.title}</li>)}</ul>;
}
