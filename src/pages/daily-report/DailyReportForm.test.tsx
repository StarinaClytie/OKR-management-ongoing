...
describe('DailyReportForm', () => {
  it('uses the shared padded card contract for O, KR, and evidence sections', () => {
    const { container } = render(<DailyReportForm objectives={objectives} keyResults={keyResults} onCancel={vi.fn()} onSubmit={vi.fn()} />);
    expect(container.querySelector('.daily-objective-field')).toHaveClass('form-card', 'form-section');
    expect(container.querySelector('.daily-key-results')).toHaveClass('form-card', 'form-section');
    expect(container.querySelector('.daily-evidence')).toHaveClass('form-card', 'form-section');
  });
