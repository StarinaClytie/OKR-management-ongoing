...
it('maps a persisted block report to an editable block draft', () => {
  const report: DailyReport = {
    ...
    blocks: [{
      id: 'block-1', dailyObjective: '完成实验采集第一阶段', projectId: 'project-1', keyResultId: 'kr-1', hours: 3.5, result: '样本已采集',
  ...
  expect(dailyReportToDraft(report)).toEqual(expect.objectContaining({
    blocks: [expect.objectContaining({
      dailyObjective: '完成实验采集第一阶段', linkedKeyResultId: 'kr-1', projectId: 'project-1', hours: 3.5, result: '样本已采集',
